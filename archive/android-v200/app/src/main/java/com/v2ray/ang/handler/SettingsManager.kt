...
object SettingsManager {
    ...
    @Volatile
    private var hevTunNativeAvailable: Boolean? = null
    ...
    fun initApp(context: Context) {
        ...
        FlowGateModeManager.ensureInitialized(context)
    }
    ...
    fun applyAppLocale(languageCode: String? = null) {
    ...
    fun isUsingHevTun(): Boolean {
        return MmkvManager.decodeSettingsBool(AppConfig.PREF_USE_HEV_TUNNEL, false) && hasHevTunNative()
    }

    private fun hasHevTunNative(): Boolean {
        hevTunNativeAvailable?.let { return it }
        return runCatching {
            System.loadLibrary("hev-socks5-tunnel")
            true
        }.getOrElse {
            LogUtil.w(AppConfig.TAG, "Hev TUN native library is unavailable; falling back to Xray TUN")
            false
        }.also {
            hevTunNativeAvailable = it
        }
    }
    ...
    private fun ensureDefaultSettings() {
        ...
        ensureDefaultValue(AppConfig.PREF_FLOW_SPEED_TEST_URL, AppConfig.FLOW_SPEED_TEST_URL)
        ensureDefaultValue(AppConfig.PREF_FLOW_SPEED_TEST_SIZE_BYTES, AppConfig.FLOW_SPEED_TEST_SIZE_BYTES.toString())
