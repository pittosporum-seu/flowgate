...
object MmkvManager {
    ...
    fun encodeServerDownloadBenchmark(guid: String, bps: Long, state: String?) {
        if (guid.isBlank()) {
            return
        }
        val aff = decodeServerAffiliationInfo(guid) ?: ServerAffiliationInfo()
        aff.downloadBps = bps
        aff.downloadTestAt = System.currentTimeMillis()
        aff.benchmarkState = state
        serverAffStorage.encode(guid, JsonUtil.toJson(aff))
    }
    ...
    fun clearAllTestDelayResults(keys: List<String>?) {
        keys?.forEach { key ->
            decodeServerAffiliationInfo(key)?.let { aff ->
                ...
                aff.downloadBps = 0
                aff.downloadTestAt = 0
                aff.benchmarkState = null
