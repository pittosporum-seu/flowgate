...
import com.v2ray.ang.handler.FlowNodeBenchmarkManager
...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private val downloadTestScope by lazy { CoroutineScope(Dispatchers.IO) }
    ...
    override fun onCleared() {
        ...
        downloadTestScope.coroutineContext[Job]?.cancelChildren()
    ...
    fun testDownloadSpeed(guid: String) {
        if (guid.isBlank()) return
        downloadTestScope.launch {
            FlowNodeBenchmarkManager.testDownload(getApplication(), guid)
            launch(Dispatchers.Main) {
    ...
    fun testDownloadSpeedForVisibleNodes() {
        testDownloadSpeedForGuids(serversCache.map { it.guid })
    }

    fun testDownloadSpeedForGuids(guids: List<String>) {
        downloadTestScope.coroutineContext[Job]?.cancelChildren()
        val guidCopy = guids.filter { it.isNotBlank() }.distinct()
        downloadTestScope.launch {
            guidCopy.forEach { guid ->
                FlowNodeBenchmarkManager.testDownload(getApplication(), guid)
                launch(Dispatchers.Main) {
                    updateListAction.value = getPosition(guid)
                }
    ...
    fun sortByBestNodeResults() {
        if (subscriptionId.isEmpty()) {
            MmkvManager.decodeSubsList().forEach { guid ->
                sortByBestNodeResultsForSub(guid)
            }
        } else {
            sortByBestNodeResultsForSub(subscriptionId)
        }
    }
    ...
    private fun sortByBestNodeResultsForSub(subId: String) {
        val serverListToSort = MmkvManager.decodeServerList(subId)
        val sortedServerList = serverListToSort.sortedWith(
            compareByDescending<String> { guid ->
                MmkvManager.decodeServerAffiliationInfo(guid)?.downloadBps ?: 0L
            }.thenBy { guid ->
