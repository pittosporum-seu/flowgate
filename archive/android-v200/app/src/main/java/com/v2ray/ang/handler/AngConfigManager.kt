...
object AngConfigManager {
    ...
    fun updateConfigViaSub(it: SubscriptionCache): SubscriptionUpdateResult {
        try {
            ...
            val (configText, fetchMessage) = fetchSubscriptionContent(url, it.subscription.userAgent)
            if (configText.isEmpty()) {
                val message = fetchMessage ?: "Failed to fetch subscription"
                FlowGateModeManager.recordLastError(message)
                return SubscriptionUpdateResult(failureCount = 1, message = message)
            }
            ...
            if (count > 0) {
                ...
                FlowGateModeManager.clearLastError()
                ...
                val message = "Subscription fetched but no valid configs parsed"
                FlowGateModeManager.recordLastError(message)
                return SubscriptionUpdateResult(failureCount = 1, message = message)
            }
        } catch (e: Exception) {
    ...
    private fun fetchSubscriptionContent(url: String, userAgent: String?): Pair<String, String?> {
        val mode = FlowGateModeManager.getSubscriptionUpdateMode()
        val attempts = when (mode) {
            FlowSubscriptionUpdateMode.AUTO -> listOf(FlowSubscriptionUpdateMode.CURRENT_PROXY, FlowSubscriptionUpdateMode.DIRECT)
            FlowSubscriptionUpdateMode.CURRENT_PROXY -> listOf(FlowSubscriptionUpdateMode.CURRENT_PROXY)
            FlowSubscriptionUpdateMode.DIRECT -> listOf(FlowSubscriptionUpdateMode.DIRECT)
        }
        ...
        attempts.forEach { attempt ->
            val content = try {
                HttpUtil.getUrlContentWithUserAgent(
                    UrlContentRequest(
                        ...
                        userAgent = userAgent,
                        timeout = 15000,
                        httpPort = if (attempt == FlowSubscriptionUpdateMode.CURRENT_PROXY) SettingsManager.getHttpPort() else 0,
                        proxyUsername = if (attempt == FlowSubscriptionUpdateMode.CURRENT_PROXY) SettingsManager.getSocksUsername() else null,
                        proxyPassword = if (attempt == FlowSubscriptionUpdateMode.CURRENT_PROXY) SettingsManager.getSocksPassword() else null
                    )
                )
            } catch (e: Exception) {
                val label = if (attempt == FlowSubscriptionUpdateMode.CURRENT_PROXY) "current proxy" else "direct"
                val message = "$label failed: ${e.message ?: e.javaClass.simpleName}"
                LogUtil.e(AppConfig.TAG, "Update subscription: $message", e)
                errors.add(message)
                ""
            }
            ...
            if (content.isNotEmpty()) {
                val message = when {
                    mode == FlowSubscriptionUpdateMode.AUTO && attempt == FlowSubscriptionUpdateMode.DIRECT && errors.isNotEmpty() ->
                        "Updated directly after proxy fallback"
                    attempt == FlowSubscriptionUpdateMode.CURRENT_PROXY -> "Updated through current proxy"
                    else -> "Updated directly"
                }
                return content to message
            }
        }

        return "" to errors.joinToString("; ").ifBlank { "Subscription update failed" }
    }
