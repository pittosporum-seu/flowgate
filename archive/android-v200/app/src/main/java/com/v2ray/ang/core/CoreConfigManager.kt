...
object CoreConfigManager {
    ...
    fun getV2rayConfig4HttpBenchmark(context: Context, guid: String, httpPort: Int): ConfigResult {
        try {
            val configContext = CoreConfigContextBuilder.build(context, guid)
                ?: return ConfigResult(status = false, guid = guid, errorMessage = "Failed to build config context")
            if (configContext.resolvedType == CoreResolvedType.CUSTOM) {
            ...
            LogUtil.e(AppConfig.TAG, "Failed to get V2ray config for download benchmark", e)
            return ConfigResult(
                status = false,
                guid = guid,
                errorMessage = "Failed to get download benchmark config: ${e.message ?: e.javaClass.simpleName}"
            )
        }
    }
        ...
        if (SettingsManager.canUseProcessRouting()) {
            ...
            rule.process?.let { processList ->
                if (processList.isNotEmpty()) {
                    ...
                    if (uids.isEmpty() && isProcessOnlyRule(rule)) {
                        LogUtil.w(AppConfig.TAG, "Skipping process-only rule '${item.remarks}' because no package UID was resolved")
                        return
                    }
            ...
            if (isProcessOnlyRule(rule)) {
                LogUtil.w(AppConfig.TAG, "Skipping process-only rule '${item.remarks}' because process routing is unavailable")
                return
            }
            rule.process = null
        }
    ...
    private fun isProcessOnlyRule(rule: V2rayConfig.RoutingBean.RulesBean): Boolean {
        return !rule.process.isNullOrEmpty()
                && rule.ip.isNullOrEmpty()
                && rule.domain.isNullOrEmpty()
                && rule.port.isNullOrBlank()
                && rule.network.isNullOrBlank()
                && rule.protocol.isNullOrEmpty()
                && rule.inboundTag.isNullOrEmpty()
                && rule.source.isNullOrEmpty()
                && rule.user.isNullOrEmpty()
                && rule.attrs.isNullOrBlank()
    }
